...
All shared UI components are in the `@mercedes-benz/ui` package:
- Source: `web/libs/ui/src/lib/`
- Import: `import { Button, Badge } from '@mercedes-benz/ui';`

### Component Discovery

Browse available components in `@mercedes-benz/ui`:
- **Source**: `web/libs/ui/src/lib/`
- **Storybook**: Run `yarn nx storybook storybook` (port 4400)

**Critical**: Always check `@mercedes-benz/ui` before creating new components. Use `Message` for informational boxes, `EmptyState` for empty states, `Button` instead of `<button>`.

### Import Patterns

```tsx
// UI components
import { Button, Badge, Message } from '@mercedes-benz/ui';

// Icons
import { IconCheck, IconCross } from '@mercedes-benz/icons';

// Core utilities
import { cn } from '@mercedes-benz/core';
```

### shadcn/ui Integration

Some components built on shadcn/ui. Always import via `@mercedes-benz/ui`, never from `/src/shad/`.
...
```tsx
import { getTokenColor } from '@mercedes-benz/ui';
...
- `@mercedes-benz/ui` components: kebab-case (`button.tsx`, `empty-state.tsx`)
...
Before creating new components, check `@mercedes-benz/ui` and Storybook.
...
- `@mercedes-benz/ui`: kebab-case (`button.tsx`)
...
```tsx
import { Button, Message, EmptyState } from '@mercedes-benz/ui';
import { IconCheck } from '@mercedes-benz/icons';
import { cn } from '@mercedes-benz/core';
```
