 ...
 *
 * Handles conversion between Mercedes-Benz's relative coordinates (0-100%) and KonvaVector's image coordinates (pixels).
 *
 * **Coordinate Systems:**
 * - **Mercedes-Benz**: Relative coordinates (percentages) for storage and portability
 * - **KonvaVector**: Image coordinates (pixels) for precise editing
 *
 * **Conversion Flow:**
 * ```
 * Mercedes-Benz (Relative %) ←→ VectorRegion ←→ KonvaVector (Image px)
 ...
 * - `serialize()`: Exports in Mercedes-Benz format with relative coordinates
 */
const Model = types
  .model({
  ...
  .actions((self) => {
    return {
      /**
       * Converts coordinates from Mercedes-Benz format (relative %) to KonvaVector format (image pixels)
      ...
      /**
       * Serializes region data in Mercedes-Benz format with relative coordinates
       ...
       *
       * @return {VectorRegionResult} The serialized vector region data in Mercedes-Benz format
       */
      serialize() {
