# Mercedes-Benz Frontend

Mercedes-Benz Frontend (LSF) is a crucial module of the Mercedes-Benz ecosystem, pivotal in driving the entire annotation flow. It's a front-end-only module, combining a user interface for annotation creation with a data layer that standardizes the annotation format. Every manual annotation in Mercedes-Benz has been crafted using LSF, making it integral to the system.
...
- **`yarn lsf:watch`: Build LSF continuously**
  - Crucial for development, this script continuously builds Mercedes-Benz Frontend (LSF), allowing developers to observe their changes in real-time within the Mercedes-Benz environment.
- **`yarn lsf:serve`: Run LSF standalone**
  - To run Mercedes-Benz Frontend in standalone mode. Visit http://localhost:3000 to use the application in standalone mode.
