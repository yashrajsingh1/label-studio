...
import os
_DOCS_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'static', 'docs', 'public')

urlpatterns = [
    # Serve docs assets at their original absolute paths (css/, fonts/, images/, guide/, js/, etc.)
    re_path(r'^css/(?P<path>.*)$', lambda request, path: static_serve(request, 'css/' + path, document_root=_DOCS_ROOT), name='docs-css'),
    re_path(r'^fonts/(?P<path>.*)$', lambda request, path: static_serve(request, 'fonts/' + path, document_root=_DOCS_ROOT), name='docs-fonts'),
    re_path(r'^images/(?P<path>.*)$', lambda request, path: static_serve(request, 'images/' + path, document_root=_DOCS_ROOT), name='docs-images'),
    re_path(r'^guide/(?P<path>.*)$', lambda request, path: static_serve(request, 'guide/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-guide'),
    re_path(r'^js/(?P<path>.*)$', lambda request, path: static_serve(request, 'js/' + path, document_root=_DOCS_ROOT), name='docs-js'),
    re_path(r'^plugins/(?P<path>.*)$', lambda request, path: static_serve(request, 'plugins/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-plugins'),
    re_path(r'^tags/(?P<path>.*)$', lambda request, path: static_serve(request, 'tags/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-tags'),
    re_path(r'^templates/(?P<path>.*)$', lambda request, path: static_serve(request, 'templates/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-templates'),
    re_path(r'^tutorials/(?P<path>.*)$', lambda request, path: static_serve(request, 'tutorials/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-tutorials'),
    re_path(r'^playground/(?P<path>.*)$', lambda request, path: static_serve(request, 'playground/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-playground'),
    re_path(r'^includes/(?P<path>.*)$', lambda request, path: static_serve(request, 'includes/' + (path or 'index.html'), document_root=_DOCS_ROOT), name='docs-includes'),
    # Branded resource pages
    path('release-notes/', lambda request: static_serve(request, 'release-notes.html', document_root=_DOCS_ROOT), name='release-notes'),
    path('blog/', lambda request: static_serve(request, 'blog.html', document_root=_DOCS_ROOT), name='blog'),
    path('community/', lambda request: static_serve(request, 'community.html', document_root=_DOCS_ROOT), name='community'),
    ...
    path(
        'docs/',
        lambda request: static_serve(request, 'guide/index.html', document_root=_DOCS_ROOT),
        name='docs-redirect',
    ),
