## Mercedes-Benz JSON format of annotated tasks 

When you annotate data, Mercedes-Benz stores the output in JSON format. The raw JSON structure of each completed task uses the following example: 
...
| project | Identifier for a specific project in Mercedes-Benz. |
...
| annotations.completed_by | User ID of the user that created the annotation. Matches the list order of users on the People page on the Mercedes-Benz UI. See [Specifying annotators during import](#specifying-annotators-during-import) for import format options. |
...
When importing annotations to Mercedes-Benz, you can control which users are assigned as annotators by using the `completed_by` field within the annotation object:
...
Note that this applies when importing via the Mercedes-Benz UI, the API, or the SDK. 
