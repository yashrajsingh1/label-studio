---
title: Import pre-annotated data into Mercedes-Benz
short: Import pre-annotations
type: guide
tier: all
order: 163
order_enterprise: 163
meta_title: Import pre-annotated data into Mercedes-Benz
meta_description: Import predicted labels, predictions, pre-annotations, or pre-labels into Mercedes-Benz for your data labeling, machine learning, and data science projects.
section: "Import & Export"
parent: "tasks"
parent_enterprise: "tasks"
---

If you have predictions generated for your dataset from a model, either as pre-annotated tasks or pre-labeled tasks, you can import the predictions with your dataset into Mercedes-Benz for review and correction. Mercedes-Benz automatically displays the pre-annotations that you import on the Labeling page for each task. 

You can import pre-annotated tasks into Mercedes-Benz [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). 

!!! note 
    To generate interactive pre-annotations with a machine learning model while labeling, see [Set up machine learning with Mercedes-Benz](ml.html).

<div class="opensource-only">

!!! error enterprise
    Mercedes-Benz Enterprise customers have access to a powerful tool called Prompts. With Prompts, you can leverage LLMs to automatically generate predictions for your Mercedes-Benz tasks. For more information, see [Prompts](https://docs.mercedes-benz.com/guide/prompts_overview).
...
!!! info Tip
    With Prompts, you can leverage LLMs to automatically generate predictions for your Mercedes-Benz tasks. For more information, see [Prompts](prompts_overview).

</div>


## Prepare pre-annotations for Mercedes-Benz 

To import predicted labels into Mercedes-Benz, you must use the [Basic Mercedes-Benz JSON format](tasks.html#Basic-Label-Studio-JSON-format) and set up your tasks with the `predictions` JSON key. The Mercedes-Benz ML backend also outputs tasks in this format. Check this common video tutorial showing how to convert a submitted annotation to a prediction:
...
To format pre-annotations for Mercedes-Benz not represented in these examples, refer to the sample results JSON for the relevant object and control tags for your labeling configuration, such as the [Audio tag](/tags/audio.html) for audio classification tasks. Each tag must be represented in the JSON pre-annotations format to render predictions in the Mercedes-Benz UI. Not all object and control tags list sample results JSON. 

You can also use the [Mercedes-Benz Playground](/playground) to preview the output JSON for a specific labeling configuration.
...
For image pre-annotations, Mercedes-Benz expects the x, y, width, and height of image annotations to be provided in percentages of overall image dimension. See [Units for image annotations](predictions.html#Units_for_image_annotations) on this page for more about how to convert formats.
...
Save this example JSON as a file to import it into Mercedes-Benz, for example, `example_prediction_task.json`.
...
Import pre-annotated tasks into Mercedes-Benz [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). 

In the Mercedes-Benz UI, the imported prediction for this task looks like the following: 
<img src="../images/predictions_loaded.png" alt="screenshot of the Mercedes-Benz UI showing an image of airplanes with bounding boxes covering each airplane." style="max-width: 600px;" class="gif-border">
...
Import pre-annotated tasks into Mercedes-Benz [using the UI](tasks.html#Import-data-from-the-Label-Studio-UI) or [using the API](/api#operation/projects_import_create). In the Mercedes-Benz UI, the imported prediction for the first task looks like the following:

<center><img src="../images/predictions_loaded_text.png" alt="screenshot of the Mercedes-Benz UI showing the text with highlighted text labels and prediction scores visible." style="max-width: 100%;"></center>
...
If you want to import pre-annotations for brush mask image segmentation using the [BrushLabels tag](/tags/brushlabels.html), you must convert the masks to RLE format first. The [Mercedes-Benz Converter](https://github.com/Mercedes-Benz/label-studio-converter) package has some helper functions for this. See the following for common conversion cases and guidance.

Install Mercedes-Benz Converter:
```
...
[`brush.contour2rle(contours, contour_id, img_width, img_height)`](https://github.com/Mercedes-Benz/label-studio-converter/blob/master/label_studio_converter/brush.py#L310).

- To convert an image from path (jpg, png. bmp), use 
[`brush.image2rle(path)`](https://github.com/Mercedes-Benz/label-studio-converter/blob/master/label_studio_converter/brush.py#L343).

- To prepare the pre-annotation, use 
[`brush.image2annotation(path, label_name, from_name, to_name, ground_truth=False, model_version=None, score=None)`](https://github.com/Mercedes-Benz/label-studio-converter/blob/master/label_studio_converter/brush.py#L361)

For more assistance, review this [example code creating a Mercedes-Benz task with pre-annotations](https://github.com/Mercedes-Benz/label-studio-converter/blob/master/tests/test_brush.py#L11) for brush labels.
...
!!! note 
    The image data in this example task references an uploaded file, identified by the source_filename assigned by Mercedes-Benz after uploading the image. The best way to reference image data is using presigned URLs for images stored in cloud storage, or absolute paths to image data stored in local storage and added to Mercedes-Benz by [syncing storage](storage.html). 
...
See [Troubleshooting ML Backends & Predictions](https://support.mercedes-benz.com/hc/en-us/sections/23627938255117-ML-Backend-Predictions) in the Mercedes-Benz support center.
