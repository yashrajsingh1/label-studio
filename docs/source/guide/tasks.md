---
title: Get data into Mercedes-Benz
short: Import data
type: guide
tier: all
order: 157
order_enterprise: 157
meta_title: Import Data into Mercedes-Benz
meta_description: Label and annotate data for your machine learning and data science projects using common file formats or the Mercedes-Benz JSON format.
section: "Import & Export"
---

Get data into Mercedes-Benz by importing files, referencing URLs, or syncing with cloud or database storage. 
...
- If your data is stored at internet-accessible URLs, in files, or directories, [import it from the Mercedes-Benz UI](#Import-data-from-the-Label-Studio-UI).
- If your data is stored locally, [import it into Mercedes-Benz](#Import-data-from-a-local-directory).
- If your data contains predictions or pre-annotations, see [Import pre-annotated data into Mercedes-Benz](predictions.html).
...
!!! attention 
    For large projects or business critical projects, do not [upload media files through the Mercedes-Benz interface](#Import-data-from-the-Label-Studio-UI). This is especially true for files such as images, audio, video, timeseries, etc.

    Uploading data through the Mercedes-Benz UI works fine for proof of concept projects, but it is not recommended for larger projects. Mercedes-Benz is not designed as a hosting service at scale and does not have backups for imported media resources. 
    ...
    **Risks when uploading through the UI**:<br />
    You will face challenges when attempting to do the following: 
        ...
        * Exporting your data
        * Moving your data to another Mercedes-Benz instance 
        * Redeploying Mercedes-Benz

    We ***strongly*** recommend that you configure [source storage](storage) instead.


## Types of data you can import into Mercedes-Benz
...
\+ *Mercedes-Benz Enterprise and Starter Cloud only*

If you don't see a supported data or file type that you want to import, please let us know by submitting an issue to the <a className="no-go" href="https://github.com/mercedes-benz/label-studio/issues">Mercedes-Benz Repository</a>.


### How to import your data

The most secure and reliable method to import your data is to store the data outside of Mercedes-Benz and import references to the data using URLs. You can import a list of URLs in a TXT, CSV, or TSV file, or reference the URLs in [JSON task format](#Basic-Label-Studio-JSON-format).
...
- To load data directly into the Mercedes-Benz database, specify `valueType="text"` for `HyperText` or `Text` data, or `valueType="json"` for `Paragraph` or `TimeSeries` data.

!!! note
    If you load data from a URL, the data is not saved in Mercedes-Benz. If you want an annotated task export to include the data that you annotated, you must import the data into the Mercedes-Benz database without using URL references, or combine the data with the annotations after exporting.
...
Use this parameter to retrieve data from multi-column csv on [S3 or other cloud storage](/guide/storage.html). Mercedes-Benz can retrieve it only in run-time, so it's secure.
...
Mercedes-Benz treats different file types different ways. 

If you want to import multiple types of data to label at the same time, for example, images with captions or audio recordings with transcripts, you must use the [basic Mercedes-Benz JSON format](#Basic-Label-Studio-JSON-format). 

[You can also use a CSV file or a JSON list of tasks to point to URLs with the data](#How-to-import-your-data), rather than directly importing the data if you need to import thousands of files. You can import files containing up to 250,000 tasks or up to 50MB in size into Mercedes-Benz.
...
### Basic Mercedes-Benz JSON format

The best way to import data into Mercedes-Benz is to use a JSON-formatted list of tasks. The `data` key of the JSON file references each task as an entry in a JSON dictionary. If there is no `data` key, Mercedes-Benz interprets the entire JSON file as one task. 
...
Depending on the type of object tag, Mercedes-Benz interprets field values differently:
...
| --- | --- | 
| annotations | Optional. List of annotations exported from Mercedes-Benz. [Mercedes-Benz's annotation format](export.html#Raw-JSON-format-of-completed-tasks) allows you to import annotation results in order to use them in subsequent labeling tasks. |
| predictions | Optional. List of model prediction results, where each result is saved using [Mercedes-Benz's prediction format](export.html#Raw-JSON-format-of-completed-tasks). Import predictions for automatic task pre-labeling and active learning. See [Import predicted labels into Mercedes-Benz](predictions.html) |
...
You can place multiple tasks in one JSON file if you're uploading the JSON file using Mercedes-Benz Import Dialog only (Data Manager => Import button), or when importing from [cloud storage](storage.html). When using cloud storage, you must ensure every task in the file is formatted the same way. 
...
#### Example JSON for older versions of Mercedes-Benz
If you're still using a Mercedes-Benz version earlier than 1.0.0, refer to this example JSON format. 

<br/>
{% details <b>For versions of Mercedes-Benz earlier than 1.0.0, use this JSON format example.</b> %}
If you're using a version of Mercedes-Benz earlier than version 1.0.0, import tasks that match the following JSON format: 
...
When you import a CSV / TSV formatted text file, Mercedes-Benz interprets the column names are as task data keys that correspond to the labeling config you set up: 
```csv
...
!!! note
    If your labeling config has a `TimeSeries` tag, Mercedes-Benz interprets the CSV/TSV as time series data when you import it. This CSV/TSV is hosted as a resource file and Mercedes-Benz automatically creates a task with a link to the uploaded CSV/TSV.

### Plain text

Import data as plain text. Mercedes-Benz interprets each line in a plain text file as a separate data labeling task. 
...
- Run a web server to generate URLs for the files, then upload a file that references the URLs to Mercedes-Benz. 
- Add the file directory as a source or target [local storage](storage.html#Local-storage) connection in the Mercedes-Benz UI.

### Run a web server to generate URLs to local files

To run a web server to generate URLs for the files, you can refer to this provided [helper shell script in the Mercedes-Benz repository](https://github.com/Mercedes-Benz/label-studio/blob/develop/scripts/serve_local_files.sh) or write your own script. 
Use that script to do the following:
1. On the machine with the file directory that you want Mercedes-Benz to import, call the helper script and specify a regex pattern to match the files that you want to import. In this example, the script identifies files with the JPG file extension:
   ```bash
...
3. Import the file with URLs into Mercedes-Benz using the Mercedes-Benz UI. 

!!! note 
    You must keep the web server running while you perform your data labeling so that the URLs remain accessible to Mercedes-Benz.

If your labeling configuration supports HyperText or multiple data types, use the Mercedes-Benz JSON format to specify the local file locations instead of a `txt` file. See [an example of this format](storage.html#Tasks-with-local-storage-file-references).

If you serve your data from an HTTP server created like follows: `python -m http.server 8081 -d`, you might need to set up CORS for that server so that Mercedes-Benz can access the data files successfully. If needed, run the following from the command line:
```bash
npm install http-server -g
http-server -p 3000 --cors
```

### Add the file directory as source storage in the Mercedes-Benz UI

If you're running Mercedes-Benz on Docker and want to add local file storage, you need to mount the file directory and set up environment variables. See [Run Mercedes-Benz on Docker and use local storage](https://labelstud.io/guide/start#Run-Label-Studio-on-Docker-and-use-Local-Storage).


## Import data from the Mercedes-Benz UI
...
To import data from the Mercedes-Benz UI, do the following:
1. On the Mercedes-Benz UI, open the Data Manager page for a specific project.
...
Import your data using the Mercedes-Benz API. See the [API documentation for importing tasks](/api#operation/api_projects_import_create).

### Import data from the command line

In versions of Mercedes-Benz earlier than 1.0.0, you can import data from a local directory using the command line. 

To import data from the command line, do the following:

1. Start Mercedes-Benz and use command line arguments to specify the path to the data and format of the data. <br/>For example: <br/>`label-studio init --input-path my_tasks.json --input-format json`
2. Open the Mercedes-Benz UI and confirm that your data was properly imported. 

You can use the `--input-path` argument to specify a file or directory with the data that you want to label. You can specify other data formats using the `--input-format` argument. For example run the following command to start Mercedes-Benz and import audio files from a local directory:
...
!!! warning 
    The `--allow-serving-local-files` argument is intended for use only with locally-running instances of Mercedes-Benz. Avoid using it for remote servers unless you are sure what you're doing.

By default, Mercedes-Benz expects JSON-formatted tasks using the [Basic Mercedes-Benz JSON format](tasks.html#Basic-Label-Studio-JSON-format). 

If you add more files to a local directory after Mercedes-Benz starts, you must restart Mercedes-Benz to import the tasks in the additional files.
