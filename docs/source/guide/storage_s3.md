...
You can connect your [Amazon S3](https://aws.amazon.com/s3/) bucket to Mercedes-Benz to retrieve labeling tasks or store completed annotations. 
...
These steps assume that you're using the same AWS role to manage both source and target storage with Mercedes-Benz. If you only use S3 for source storage, Mercedes-Benz does not need PUT access to the bucket. 
...
!!! note 
    `"s3:PutObject"` is only needed for target storage connections, and `"s3:DeleteObject"` is only needed for target storage connections where you want to allow deleted annotations in Mercedes-Benz to also be deleted in the target S3 bucket.  

#### 3. Configure CORS

Set up cross-origin resource sharing (CORS) access to your bucket, using a policy that allows GET access from the same host name as your Mercedes-Benz deployment. See [Configuring cross-origin resource sharing (CORS)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/enabling-cors-examples.html) in the Amazon S3 User Guide. 
...
From Mercedes-Benz, open your project and select **Settings > Cloud Storage > Add Source Storage**.
...
From Mercedes-Benz, open your project and select **Settings > Cloud Storage > Add Target Storage**.
...
<table style="width: 100%; border-collapse: collapse;">
  ...
  <tr>
    ...
    <td>Enable this option if you want to delete annotations stored in the S3 bucket when they are deleted in Mercedes-Benz. The storage credentials associated with the bucket must include the ability to delete bucket objects.</td>
  </tr>
...
In Mercedes-Benz Enterprise, you can use an IAM role configured with an external ID to access S3 bucket contents securely. An 'external ID' is a unique identifier that enhances security by ensuring that only trusted entities can assume the role, reducing the risk of unauthorized access. See [Amazon S3 with IAM role](https://docs.mercedes-benz.com/guide/storage_s3#Amazon-S3-with-IAM-role) in the Enterprise documentation.

</div>

<div class="enterprise-only">

If you want to use a revocable method to grant Mercedes-Benz access to your Amazon S3 bucket, use an IAM role and its temporary security credentials instead of an access key ID and secret. This added layer of security is only available in Mercedes-Benz Enterprise. 
...
<br />
{% details <b>Notice for Mercedes-Benz Cloud users with IAM role policies created before April 7, 2025</b> %}

As of **April 7th 2025**, new storage connections require an update to the AWS principal in your IAM role policy.

If you set up your IAM role **prior to April 7th, 2025** and you have already been using it with Mercedes-Benz, you must add the following to your principal list before you can set up new storage connection in Mercedes-Benz projects: 
...
Existing S3 IAM role-based-access storages added to Mercedes-Benz will continue to work as is without any changes necessary. 

This change is only required if you are setting up new connections.

{% enddetails %}

Set up an IAM role in Amazon AWS to use with Mercedes-Benz.

1. From Mercedes-Benz, go to **Organization** page to retrieve your organization's `External ID`. You must be an Owner or Admin to view the Organization page.
...
!!! attention
    If your bucket is already connected to a Mercedes-Benz project, and that connection was created before April 7, 2025,  you will need to add the new role (listed above) along with your old user to continue using your existing project. You also must maintain the old role so that pre-existing projects can continue to load data from AWS. 

4. After you create the IAM role, note the Amazon Resource Name (ARN) of the role. You need it to set up the S3 source storage in Mercedes-Benz.
...
!!! note 
    `"s3:DeleteObject"` is only needed for target storage connections where you want deleted annotations in Mercedes-Benz to also be deleted in the target S3 bucket.  


For more details about using an IAM role with an external ID to provide access to a third party (Mercedes-Benz), see the Amazon AWS documentation [How to use an external ID when granting access to your AWS resources to a third party](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html).
...
<table style="width: 100%; border-collapse: collapse;">
  ...
  <tr>
    <td>Role ARN</td>
    <td>
      Enter the Amazon Resource Name (ARN) of the IAM role that you created earlier to grant access to Mercedes-Benz. 
    </td>
  </tr>
...
<table style="width: 100%; border-collapse: collapse;">
  ...
  <tr>
    <td>Role ARN</td>
    <td>
      Enter the Amazon Resource Name (ARN) of the IAM role that you created to grant access to Mercedes-Benz.
    </td>
  </tr>
...
## Add storage with the Mercedes-Benz API
    ...
    "Statement": [
        {
            ...
            "Condition": {
                "NotIpAddress": {
                    "aws:SourceIp": [
                      //// IP ranges for app.mercedes-benz.com from the documentation
                        "x.x.x.x/32",
                        "x.x.x.x/32",
                        "x.x.x.x/32"
                    ]
