---
title: Set up SSO authentication for Mercedes-Benz
short: SSO and SAML
tier: enterprise
type: guide
order: 0
order_enterprise: 387
meta_title: SSO authentication for Mercedes-Benz Enterprise
meta_description: Mercedes-Benz Enterprise documentation for setting up SSO authentication for your data labeling, machine learning, and data science projects.
section: "Manage Your Organization"
parent: "admin_auth"
parent_enterprise: "admin_auth"
---

Set up single sign-on using SAML to manage access to Mercedes-Benz using your existing Identity Provider (IdP).

!!! error Enterprise
    SSO authentication is only available in Mercedes-Benz Enterprise Edition. If you're using Mercedes-Benz Community Edition, see <a href="https://labelstud.io/guide/label_studio_compare.html">Mercedes-Benz Features</a> to learn more.


To more easily [manage access to Mercedes-Benz Enterprise](manage_users.html), you can map SAML groups to both `roles` or `workspaces`. 

## Set up SAML SSO

The organization Owner or Administrator for Mercedes-Benz Enterprise can set up SSO & SAML for the instance. Mercedes-Benz Enterprise supports the following IdPs:
...
After setting up the SSO, you can use native authentication to access the Mercedes-Benz UI, however it's not a recommended option especially for the user with the Owner role.
...
### Connect your Identity Provider to Mercedes-Benz Enterprise

Set up Mercedes-Benz Enterprise as a Service Provider (SP) with your Identity Provider (IdP) to use SAML authentication. 
...
4. Copy the following URLs:
    ...
    * **Login URL**---This is the URL that users will use to log in to Mercedes-Benz. 
    * **Logout URL**---This is the URL used to redirect users after successfully logging out of Mercedes-Benz.

#### From your IdP:

1. Paste the URLs copied from Mercedes-Benz in the appropriate location. 
2. Generate a metadata XML file, or a URL that specifies the metadata for the IdP.
3. Set up or confirm setup of the following SAML attributes. Mercedes-Benz Enterprise expects specific attribute mappings for user identities.
...
!!! note Note
    Different Identity Providers use different attribute names. Mercedes-Benz provides **presets** in the SSO & SAML settings page to quickly configure the correct attribute mappings for popular IdPs. You can also manually configure custom attribute names if your IdP uses different values.
...
3. Set up group mappings. These can also be added or edited later.
    ...
    * **Projects to Groups Mapping**---Map groups to roles at the project level. Project-level roles can be **Annotator**, **Reviewer**, or **Inherit**. 
        ...
        You can map a group to different roles across multiple projects. You can also map multiple groups to the same roles and the same projects. For more information on roles, see [Roles in Mercedes-Benz Enterprise](manage_users#Roles-in-Label-Studio-Enterprise). 
...
5. Test the configuration by logging in to Mercedes-Benz Enterprise with your SSO account.
...
If you want to manage Mercedes-Benz roles and workspaces entirely with single sign-on (SSO), add the following to your environment variable file:
...
Setting these options disables the Mercedes-Benz API and UI options to assign roles and workspaces for specific users within Mercedes-Benz and relies entirely on the settings in the environment variable file.

!!! info Tip
    If you are using the SaaS version of Mercedes-Benz (Mercedes-Benz Enterprise Cloud) and would like to enable these restrictions for your organization, [open a ticket](https://support.mercedes-benz.com/hc/en-us/requests/new) to submit your request.  
