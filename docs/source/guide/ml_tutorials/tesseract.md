...
meta_title: Interactive bounding boxes OCR in Mercedes-Benz with a Tesseract backend
meta_description: Tutorial for how to use Mercedes-Benz and Tesseract to assist with your OCR projects
categories:
    - Computer Vision
...
Tested against Mercedes-Benz 1.10.1, with basic support for both Mercedes-Benz
...
You must also install the [Mercedes-Benz ML backend](https://github.com/Mercedes-Benz/label-studio-ml-backend?tab=readme-ov-file#quickstart). 

This tutorial uses the [`tesseract` example](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/tesseract). 

### 1. Install Mercedes-Benz

Launch Mercedes-Benz. You can follow the guide from the [official documentation](https://labelstud.io/guide/install.html) or use the following commands:
   ...
   Optionally, you may enable local file serving in Mercedes-Benz
   ...
   Mercedes-Benz to connect the model.

### 2. Create a Mercedes-Benz project
...
Download the Mercedes-Benz Machine Learning backend repository.
   ```
...
If you opted to use Mercedes-Benz Local File Storage, be sure to set the `LABEL_STUDIO_HOST` and `LABEL_STUDIO_ACCESS_TOKEN` variables. 
...
### 5. Upload tasks
   ...
   If you're using the Mercedes-Benz Local File Storage option, upload images
   directly to Mercedes-Benz using the Mercedes-Benz interface.
