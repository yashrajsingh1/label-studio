---
title: YOLO ML Backend for Mercedes-Benz
type: guide
tier: all
order: 50
hide_menu: true
hide_frontmatter_title: true
meta_title: YOLO ML Backend for Mercedes-Benz
meta_description: Tutorial on how to use an example ML backend for Mercedes-Benz with YOLO
categories:
    - Computer Vision
    - Object Detection
    - Image Segmentation
...
# YOLO ML backend for Mercedes-Benz

The YOLO ML backend for Mercedes-Benz is designed to integrate advanced object detection, 
segmentation, classification, and video object tracking capabilities directly into Mercedes-Benz. 
...
| [Video Temporal Classification](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/README_TIMELINE_LABELS.md) | `<TimelineLabels>`                   | ✅                    | Native              | Native              |


* **LS Control Tag**: Mercedes-Benz [control tag](https://labelstud.io/tags/) from the labeling configuration. 
* **LS Import Supported**: Indicates whether Mercedes-Benz supports Import from YOLO format to Mercedes-Benz (using the LS converter).
* **LS Export Supported**: Indicates whether Mercedes-Benz supports Export from Mercedes-Benz to YOLO format (the **Export** button on the Data Manager and using the LS converter).
* **Native**: Native means that only native Mercedes-Benz JSON format is supported.


## Before you begin 

Before you begin, you need to install the [Mercedes-Benz ML backend](https://github.com/Mercedes-Benz/label-studio-ml-backend?tab=readme-ov-file#quickstart). 

This tutorial uses the [YOLO example](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/yolo).

## Quick start

1. Add `LABEL_STUDIO_URL` and `LABEL_STUDIO_API_KEY` to the `docker-compose.yml` file. 
These variables should point to your Mercedes-Benz instance and its API key, respectively. 
For more information about finding your Mercedes-Benz API key, [see our documentation](https://labelstud.io/guide/user_account#Access-token).
...
3. Open Mercedes-Benz and create a new project with the following labeling config:
...
5. Add images or video (depending on tasks you are going to solve) to Mercedes-Benz.
...
```mermaid
graph TD
    A[Mercedes-Benz :: Labeling Config :: Labels or Choices] <--> B[ML Model :: Names]
```
...
Or you can find some labels in [YOLO_CLASSES.md](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/YOLO_CLASSES.md)
</details>

<details>
<summary>Tip: How to map my labels to YOLO names using an LLM?</summary>
<br/>
You can use an LLM model (e.g. ChatGPT) to automatically build mapping between Mercedes-Benz labels and ML model labels. 
...
2. **Labeling Config:**
   - I have this labeling config from Mercedes-Benz:
   <View>
     <Image name="image" value="$image"/>
...
3. **Mapping Instructions:**
   - Map the labels from the Mercedes-Benz config to the closest matching ML model labels as follows:
...
<br/>
You can integrate your own custom-trained YOLOv8 models with the YOLO ML backend for Mercedes-Benz. 
Follow these detailed steps to set up your custom model in the ML backend Docker:

### Step 0: Install Mercedes-Benz and clone this Github repository

1. Install and run Mercedes-Benz (or see [more ways here](https://labelstud.io/guide/install)). 
...
2. Clone the Mercedes-Benz ML backend repository and go to the `yolo` example folder:

```
git clone https://github.com/Mercedes-Benz/label-studio-ml-backend.git
...
- **`LABEL_STUDIO_URL`**: This is necessary to specify the external IP or domain of your Mercedes-Benz instance. 
  If you run Mercedes-Benz and ML backend locally, you can try setting it to `LABEL_STUDIO_URL=http://host.docker.internal:8080`.
- **`LABEL_STUDIO_API_KEY`**: This is necessary to specify the API key of your Mercedes-Benz instance. 
  You can find it in Mercedes-Benz on the [User Account page](https://labelstud.io/guide/user_account#Access-token).
...
### Step 4: Update your Labeling Configuration in Mercedes-Benz

In your Mercedes-Benz project, specify the path to your custom model in the labeling configuration 
...
### Step 6: Connect the ML Backend to your Mercedes-Benz project

1. Open your Mercedes-Benz instance.
...
Add some tasks (images or other data) to your Mercedes-Benz project and open a task in the labeling interface. 
...
- **Label mismatch:** Double-check that your labels in Mercedes-Benz match the classes your model predicts, or use `predicted_values` to map them.
...
The current Mercedes-Benz ML backend doesn't support training YOLO models. You have to do it manually on your side.
...
YOLO provides a classification model and Mercedes-Benz supports this with the `Choices` control tag.
...
Mercedes-Benz supports this with the `RectangleLabels` control tag.
...
Mercedes-Benz supports this with the `PolygonLabels` control tag.
...
Mercedes-Benz supports this with the `KeyPointLabels` control tag.
...
You will see the grouping under the **Regions** panel on the right side of Mercedes-Benz. Note that you can drag and drop region items as necessary.
...
For precise control, you can map keypoints to specific labels in your Mercedes-Benz configuration. 
...
This configuration ensures that the keypoints detected by the YOLO model are correctly labeled in the Mercedes-Benz interface.
...
Mercedes-Benz supports this with the `VideoRectangle` + `Labels` control tags.
...
* Mercedes-Benz has timeout limits for ML backend requests. You can adjust the timeout in the Mercedes-Benz backend settings.

<!-- TODO: https://github.com/Mercedes-Benz/label-studio/pull/5414/files#diff-20432d8093df2c0400b0f41b004a6b772b856b985fa1f5fd1e1f909247c89fc6L30 -->
...
This ML backend supports temporal multi-label video classification for the [`<TimelineLabels>` control tag](https://labelstud.io/tags/timelinelabels) in Mercedes-Benz. 
...
- **Trainable:** In the [trainable mode](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/README_TIMELINE_LABELS.md), the model can be trained on custom labels and annotations submitted in Mercedes-Benz using few-shot learning as training is performed on a small number of annotations.  
...
**[README_TIMELINE_LABELS.md](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/README_TIMELINE_LABELS.md)**.
...
3. Create a project in Mercedes-Benz. Then from the **Model** page in the project settings, [connect the model](https://labelstud.io/guide/ml#Connect-the-model-to-Label-Studio). The default URL is `http://localhost:9090`.
...
Also, you can check [Dockerfile](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/Dockerfile) for additional dependencies and install them manually. 

### Parameters

Check the `environment` section in the [`docker-compose.yml`](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/docker-compose.yml) file before running the container. 
...
This Command Line Interface (CLI) tool facilitates the integration of YOLO models with Mercedes-Benz for machine learning predictions. 
It provides an alternative method for running YOLO predictions on tasks managed by Mercedes-Benz, 
...
like connection timeouts between Mercedes-Benz and the ML backend, 
...
Mercedes-Benz may interrupt the connection with the ML backend if the request takes too long, resulting in incomplete predictions. 

By running this CLI tool, you can execute model predictions asynchronously 
without the need for Mercedes-Benz to maintain a constant connection to the backend. 
This ensures that even large or complex tasks are processed fully, 
and predictions are saved to Mercedes-Benz using SDK once completed.

### How it works

1. **Mercedes-Benz Connection**: The tool connects to a running instance of Mercedes-Benz using the provided API key and URL.
2. **Task Preparation**: Tasks can be provided directly via a JSON file or as a list of task IDs. The tool fetches task data from Mercedes-Benz if task IDs are supplied.
3. **Model Loading**: The YOLO model is loaded and initialized based on the project’s configuration.
4. **Prediction Process**: For each task, the YOLO model generates predictions, which are then post-processed to Mercedes-Benz's expected format.
5. **Asynchronous Upload**: The generated predictions are uploaded back to Mercedes-Benz, allowing for large tasks to be processed without timing out.
...
- **`--ls-url`**: The URL of the Mercedes-Benz instance. Defaults to `http://localhost:8080`.
- **`--ls-api-key`**: The API key for Mercedes-Benz. Used to authenticate the connection.
- **`--project`**: The ID of the Mercedes-Benz project where the tasks are managed. Defaults to `1`.
- **`--tasks`**:
...
The architecture of the project and development guidelines are described in the [README_DEVELOP.md](https://github.com/Mercedes-Benz/label-studio-ml-backend/blob/master/label_studio_ml/examples/yolo/README_DEVELOP.md) file.
