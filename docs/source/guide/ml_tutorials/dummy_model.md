...
meta_description: Mercedes-Benz tutorial for creating and integrating your Machine Learning backend with Mercedes-Benz.
...
This tutorial explains the basics of using a Machine Learning (ML) backend with Mercedes-Benz. For the sake of simplicity, this tutorial relies on a _dummy model_ that just produces random predictions.
...
If you create an ML backend using [Mercedes-Benz's ML SDK](/guide/ml_create.html), make sure your ML backend script does the following:

- Inherit the created model class from `label_studio_ml.LabelStudioMLBase`
- Override the 2 methods:
    - `predict()`, which takes [input tasks](/guide/tasks.html#Basic-Label-Studio-JSON-format) and outputs [predictions](/guide/predictions.html) in the Mercedes-Benz JSON format.
...
Mercedes-Benz can automatically create all necessary configs and scripts needed to run ML backend from your newly created model.
...
### Using ML backend with Mercedes-Benz

Initialize and start a new Mercedes-Benz project connecting to the running ML backend:
...
You should see model predictions in a labeling interface. See [Set up machine learning with Mercedes-Benz](/guide/ml.html).
