---
title: Time Series Segmenter for Mercedes-Benz
type: guide
tier: all
order: 51
hide_menu: true
hide_frontmatter_title: true
meta_title: Time Series Segmenter for Mercedes-Benz
meta_description: Tutorial demonstrating a minimal ML backend that performs time series segmentation in Mercedes-Benz.
categories:
    - Time Series
    - Segmentation
image: "/guide/ml_tutorials/timeseries.png"
---
# Time Series Segmenter for Mercedes-Benz
...
1. Install the [Mercedes-Benz ML backend](https://github.com/Mercedes-Benz/label-studio-ml-backend?tab=readme-ov-file#quickstart).
...
You can use [the following data](https://raw.githubusercontent.com/Mercedes-Benz/label-studio-ml-backend/refs/heads/master/label_studio_ml/examples/timeseries_segmenter/tests/test_from_video_tutorial.csv) for tests with this labeling configuration.
...
1. **Data Collection**: Fetches all labeled tasks from your Mercedes-Benz project
...
The model processes new time series data by applying the trained LSTM classifier with sliding window temporal context. Only meaningful event segments are returned to Mercedes-Benz, filtering out background periods automatically.
...
5. **Result Formatting**: Returns segments in Mercedes-Benz JSON format with proper instant field values
...
The backend automatically handles multiple Mercedes-Benz projects by maintaining separate trained models for each project. This ensures proper isolation and prevents cross-project interference.
...
**Scalability:**
- Backend can serve multiple Mercedes-Benz projects simultaneously
...
This seamless multi-tenant support makes the backend suitable for enterprise Mercedes-Benz deployments where multiple teams or clients need isolated ML models.
...
- **Project-Specific Models**: Each Mercedes-Benz project gets its own trained model for proper multi-tenant isolation
