...
meta_description: Mercedes-Benz tutorial for interactive LLM labeling with OpenAI, Azure, or Ollama
categories:
    - Generative AI
...
This example server connects Mercedes-Benz to [OpenAI](https://platform.openai.com/), [Ollama](https://ollama.com/),
...
This tutorial uses the [`llm_interactive` example](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/llm_interactive). 
...
3. Open a Mercedes-Benz project and go to **Settings > Model**. [Connect the model](https://labelstud.io/guide/ml#Connect-the-model-to-Label-Studio), specifying `http://localhost:9090` as the URL. 
...
6. Enter a prompt in the prompt input field and press `Shift+Enter`. The LLM response will be generated and displayed in
   the response field.
7. If you want to apply LLM auto-annotation to multiple tasks at once, go to the [Data Manager](https://labelstud.io/guide/manage_data), select a group of tasks and then select **Actions > Retrieve Predictions** (or **Batch Predictions** in Mercedes-Benz Enterprise).
