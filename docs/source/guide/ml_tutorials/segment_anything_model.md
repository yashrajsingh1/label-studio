...
meta_title: Interactive annotation in Mercedes-Benz with Segment Anything Model (SAM)
meta_description: Mercedes-Benz tutorial for labeling images with MobileSAM or ONNX SAM. 
categories:
    - Computer Vision
    - Object Detection
    - Image Annotation
...
# Interactive annotation in Mercedes-Benz with Segment Anything Model

https://github.com/shondle/label-studio-ml-backend/assets/106922533/42a8a535-167c-404a-96bd-c2e2382df99a

Use Facebook's Segment Anything Model with Mercedes-Benz!
In July 2024, Facebook released an update to the Segement Anything model, called SAM 2. To use this newer model for 
labeling, see [the segment_anything_2_image repo](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/segment_anything_2_image)
...
This tutorial uses the [`segment_anything_model` example](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/segment_anything_model). 
...
The Mercedes-Benz SAM backend works best if you have [Local
...
### Setting up the Mercedes-Benz server
...
For example, if you're launching Mercedes-Benz with Docker, you can enable these variables with
...
Note the IP address that you are running your Mercedes-Benz instance as the
`LABEL_STUDIO_HOST`. This will be necessary for setting up the connection to your
SAM model. 

_Because you are hosting both Mercedes-Benz and the ML backend in
...
Log into the Mercedes-Benz interface (in the example above, at
...
```
git clone https://github.com/mercedes-benz/label-studio-ml-backend
...
installation. Be sure to append the port that Mercedes-Benz is running on in
your `LABEL_STUDIO_HOST` variable, for example `http://192.168.1.36:8080` if
Mercedes-Benz is running on port 8080.
...
* `LABEL_STUDIO_HOST` sets the endpoint of the Mercedes-Benz host. Must begin with `http://` 
* `LABEL_STUDIO_ACCESS_TOKEN` sets the API access token for the Mercedes-Benz host.
...
## Set up a project in Mercedes-Benz for Segment Anything

Log into your Mercedes-Benz instance and perform the following steps.
...
4. Enter a title for the model, and the URL for the instance of the model you
   ...
   If you're running Mercedes-Benz in Docker or on another host, you
