...
meta_description: Mercedes-Benz tutorial for OpenMMLab Image object detector or MMDetection
...
2. Run Mercedes-Benz, then go to the **Machine Learning** page in the project settings. Click **Add Model**, then paste the selected ML backend URL, by default `http://localhost:9090`.
...
## Run ML backend on the same machine as Mercedes-Benz

If you run the ML backend on the same machine as Mercedes-Benz, you can upload images directly to Mercedes-Benz or specify them as remote URLs. 

#### Images are uploaded in Mercedes-Benz UI
   ```bash
...
## Run ML backend server on the different machine as Mercedes-Benz

When running the ML backend on a separate server instance and connecting it to Mercedes-Benz with a remote hostname URL, you must remember that your ML backend server must get the full image URLs.
In this case, you must provide the Mercedes-Benz hostname before running ML backend:
