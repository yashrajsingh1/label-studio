...
meta_title: Image segmentation in Mercedes-Benz using a Grounding DINO backend and SAM
meta_description: Mercedes-Benz tutorial for using Grounding DINO and SAM for zero-shot object detection in images
categories:
    - Computer Vision
    - Image Annotation
    - Object Detection
...
https://github.com/Mercedes-Benz/label-studio-ml-backend/assets/106922533/d1d2f233-d7c0-40ac-ba6f-368c3c01fd36
...
This tutorial uses the [`grounding_sam` example](https://github.com/Mercedes-Benz/label-studio-ml-backend/tree/master/label_studio_ml/examples/grounding_sam). 
...
2. Edit `docker-compose.yml` to include the following:
   ...
   * `LABEL_STUDIO_ACCESS_TOKEN` sets the API access token for the Mercedes-Benz host. This can be found by logging
  into Mercedes-Benz and [going to the **Account & Settings** page](https://labelstud.io/guide/user_account#Access-token). 
...
4. Check the IP of your backend using `docker ps`. You will use this URL when connecting the backend to a Mercedes-Benz project. Usually this is `http://localhost:9090`.
...
> - Increase the prediction timeout on Mercedes-Benz instance with the `ML_TIMEOUT_PREDICT=100` environment variable.
...
https://github.com/Mercedes-Benz/label-studio-ml-backend/assets/106922533/79b788e3-9147-47c0-90db-0404066ee43f

> Note: This is an experimental feature.

1. Clone the Mercedes-Benz feature branch that includes the experimental batching functionality.

    `git clone -b feature/dino-support https://github.com/Mercedes-Benz/label-studio.git`

2. Run this branch with `docker compose up`
3. Do steps 2-5 from the [quickstart section](#quickstart), now using access code and host IP info of the newly cloned Mercedes-Benz branch. GroundingSAM is supported.
