---
title: Set up active learning with Mercedes-Benz
short: Active learning loop
tier: enterprise
type: guide
order: 0
order_enterprise: 257
meta_title: Set up an active learning loop with Mercedes-Benz
meta_description: Set up an end-to-end active learning loop with Mercedes-Benz using the ML backend SDK and webhooks to perform model training and predictions and labeling.
section: "Machine Learning"

---

Follow this tutorial to set up an active learning loop with Mercedes-Benz. 

Use Mercedes-Benz Enterprise Edition to build an automated active learning loop with a machine learning model backend. If you use the open source Community Edition of Mercedes-Benz, you can manually sort tasks and retrieve predictions to mimic an active learning process. If you're using Mercedes-Benz Community Edition, see how to [manually manage your active learning loop](#Set-up-manual-active-learning).
...
2. [Connect the ML backend for getting predictions to Mercedes-Benz](#Connect-the-ML-backend-to-Label-Studio-for-active-learning).
...
As you label tasks, Mercedes-Benz sends webhook events to your machine learning backend and prompts it to retrain. As the model retrains, the predictions from the latest model version appear in Mercedes-Benz. 
...
## Connect the ML backend to Mercedes-Benz for active learning

Follow the steps to [connect a model to a Mercedes-Benz project](ml#Connect-the-model-to-Label-Studio) and ensure the setting **Start model training on annotation submission** is enabled. This sends a training request to the backend after each annotation submission or update. 

## Configure webhooks to send a training event to the ML backend (optional)

By default, Mercedes-Benz notifies your ML backend every time an annotation is created or updated so that it can start training in response. 
...
1. In the Mercedes-Benz UI, open the project that you want to use for active learning.
...
As your model retrains and a new version is updated in Mercedes-Benz, the tasks shown next to annotators are always those with the lowest prediction scores, reflecting those with the lowest model certainty. The predictions for the tasks correspond to the latest model version.
...
If you're using Mercedes-Benz Community Edition, data annotators can't experience a live active learning loop. You can mimic an active learning experience by doing the following:
