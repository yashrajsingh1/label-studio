---
title: Deploy Mercedes-Benz Enterprise on Kubernetes
short: Install using Kubernetes
tier: enterprise
type: guide
order: 0
order_enterprise: 69
meta_title: Deploy Mercedes-Benz Enterprise on Kubernetes
meta_description: Deploy Mercedes-Benz Enterprise on Kubernetes, such as on Amazon Elastic Container Service for Kubernetes, to create machine learning and data science projects in a scalable containerized environment. 
section: "Install & Setup"
parent_enterprise: "install_enterprise"

---

Deploy Mercedes-Benz Enterprise on a Kubernetes Cluster using Helm 3. You can use this Helm chart to set up Mercedes-Benz Enterprise for deployment onto a Kubernetes cluster and install, upgrade, and manage the application. 
...
!!! warning
    To install Mercedes-Benz Community Edition, see <a href="https://labelstud.io/guide/install_k8s.html">Deploy Mercedes-Benz on Kubernetes</a>. This page is specific to the Enterprise version of Mercedes-Benz.

</div>

!!! note
    On-prem deployments of Mercedes-Benz Enterprise are not supported for Academic licenses. 

This high-level architecture diagram that outlines the main components of a Mercedes-Benz Enterprise deployment.

<img src="/images/LSE_k8s_scheme.png"/>

!!! warning
    Mercedes-Benz Enterprise 2.2.9 decommissioned MinIO as a service.

Mercedes-Benz Enterprise runs on Python and uses rqworkers to perform additional tasks. Metadata and annotations are stored in a bundled version of PostgreSQL that functions as persistent storage. If you host Mercedes-Benz Enterprise in the cloud, use [persistent storage in the cloud](persistent_storage.html) instead of MinIO.

## Install Mercedes-Benz Enterprise on Kubernetes

If you want to install Mercedes-Benz Enterprise on Kubernetes and you have unrestricted access to the internet from your K8s cluster, follow these steps. 
...
10. [Use Helm to install Mercedes-Benz Enterprise on your Kubernetes cluster](#Use-Helm-to-install-Label-Studio-Enterprise-on-your-Kubernetes-cluster).

If you use a proxy to access the internet from your Kubernetes cluster, or it is airgapped from the internet, see how to [Install Mercedes-Benz Enterprise without public internet access](install_k8s_airgapped.html).
...
This chart has been tested and confirmed to work with the [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/) and [cert-manager](https://cert-manager.io/docs/). See [Set up an ingress controller for Mercedes-Benz Kubernetes deployments](ingress_config.html) for more on ingress settings with Mercedes-Benz. 
...
Mercedes-Benz Enterprise has the following default configurations for resource requests, resource limits, and replica counts:
...
Before installing Mercedes-Benz, prepare the Kubernetes cluster with [kubectl](https://kubernetes.io/docs/reference/kubectl/). 

Install Mercedes-Benz Enterprise and set up a PostgreSQL and Redis databases to store relevant Mercedes-Benz Enterprise configurations and annotations using the Helm chart. You must configure specific values for your deployment in a YAML file that you specify when installing using Helm.

### Add the Helm chart repository
Add the Helm chart repository to easily install and update Mercedes-Benz.
...
2. Create a key to pull the latest Mercedes-Benz Enterprise image from the Docker registry. From the command line of your cluster, run the following:
    ```shell
    ...
    ```
3. Create the Mercedes-Benz Enterprise license as a Kubernetes secret. You can specify it as a file or as a specific URL.
...
You must configure a `values.yaml` file for your Mercedes-Benz Enterprise deployment. The following file contains default values for a minimal installation of Mercedes-Benz. This chart has been tested and confirmed to work with the [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/) and [cert-manager](https://cert-manager.io/docs/).

Example `values.yaml` file for a minimal installation of Mercedes-Benz Enterprise Enterprise:
```yaml
global:
  image:
...
app:
  ...
  replicas: 1
  # Ingress config for Mercedes-Benz
  ingress:
    enabled: true
...
# default compute resources run Mercedes-Benz Enterprise for a basic installation. adjust according to your business needs
  resources:
    requests:
      memory: 1024Mi
      cpu: 1000m
...
To configure Mercedes-Benz Enterprise to use TLS for end-client connections with PostgreSQL, do the following:
...
4. Install or upgrade Mercedes-Benz Enterprise using Helm.

## Optional: Set up TLS for Redis
To configure Mercedes-Benz Enterprise to use TLS for end-client connections with Redis, do the following:
...
## Use Helm to install Mercedes-Benz Enterprise on your Kubernetes cluster

Use Helm to install Mercedes-Benz Enterprise on your Kubernetes cluster. Provide your custom resource definitions YAML file. Specify any environment variables that you need to set for your Mercedes-Benz Enterprise installation using the `--set` argument with the `helm install` command.

!!! note
    If you are deploying to a production environment, you should set the `SSRF_PROTECTION_ENABLED: true` environment variable. See [Secure Mercedes-Benz](security#Enable-SSRF-protection-for-production-environments).
...
## Restart Mercedes-Benz Enterprise using Helm

Restart your Helm release by doing the following from the command line:

1. Identify the &lt;RELEASE_NAME&gt; of the latest Mercedes-Benz Enterprise release:
```shell
helm list
```
2. Restart the rqworker for Mercedes-Benz:
```shell
kubectl rollout restart deployment/<RELEASE_NAME>-ls-rqworker
```
3. Restart the Mercedes-Benz Enterprise app:
```shell
kubectl rollout restart deployment/<RELEASE_NAME>-ls-app
```


## Uninstall Mercedes-Benz Enterprise using Helm

To uninstall Mercedes-Benz Enterprise using Helm, delete the configuration.
