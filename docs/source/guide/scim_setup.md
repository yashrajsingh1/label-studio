---
title: Set up SCIM2 for Mercedes-Benz
short: SCIM2
tier: enterprise
type: guide
order: 0
order_enterprise: 389
meta_title: System for Cross-domain Identity Management (SCIM) for Mercedes-Benz Enterprise
meta_description: Mercedes-Benz Enterprise documentation for setting up SCIM2
...
Using a SCIM provider, you can manage access to Mercedes-Benz Enterprise workspaces, and grant roles to individual users and groups.

## Requirements

Mercedes-Benz Enterprise uses the SCIM Version 2.0 standard. 

Mercedes-Benz Enterprise follows [SCIM RFC 5741](https://datatracker.ietf.org/doc/html/rfc7644#section-3.2) and can be integrated with any access management services that support the standard.

For more information on SCIM workflows, see [How SCIM works with Mercedes-Benz Enterprise](scim_workflow). 
...
To manage access to Mercedes-Benz Enterprise, add the application to your SCIM provider (Okta). 
...
### Add Mercedes-Benz Enterprise as an application (if not complete)

1. Navigate to **Applications > Applications** in Okta. Click  **Create App Integration**. 
2. Select **SAML 2.0**. Enter an app name (for example, _Mercedes-Benz Enterprise_).
3. Under **Configure SAML**, set up the SAML integration following the steps outlined in [Set up SSO guide](auth_setup.html).
4. Make sure Mercedes-Benz Enterprise appears in the list of active applications.

### Enable SCIM provisioning

1. Navigate to **Applications > Applications** in Okta. 
2. Select **Mercedes-Benz Enterprise**.
...
|-------|-------------------|
| **SCIM connector base URL** | `https://<LABEL_STUDIO_BASE_URL>/scim/v2/` where `<LABEL_STUDIO_BASE_URL>` is the base URL of your Mercedes-Benz Enterprise instance. |
| **Unique identifier field for users** | Use `email`. Mercedes-Benz Enterprise uses email as user identifier in this field. |
...
| **HTTP Header → `Authorization: Bearer <token>`** | Enter the [Legacy token](access_tokens#Legacy-tokens) associated with the Owner account in Mercedes-Benz. <br />For Mercedes-Benz, `Token` and `Bearer` are the same tokens. However, it's important to use `Bearer` instead of `Token` in the request header. | 
...
3. Select the people you would like to be added to Mercedes-Benz Enterprise.
4. Click **Done**.

After you click **Done**, Okta will send the requests to create users accordingly in the Mercedes-Benz Enterprise.
...
The most convenient way to manage access to the application is via groups. You can assign Mercedes-Benz to groups and manage the groups in Okta. The changes will be propagated to the application.

### Set up group mapping in Mercedes-Benz

1. In Mercedes-Benz, click the menu in the upper left and select **Organization**. 
...
3. Update roles and workspaces mapping. Ensure the group name you enter is the same as the group name being sent by your SCIM provider. 
    ...
    * **Organization Roles to Groups Mapping**---Map groups to roles at the organization level. The role set at the organization level is the default role of the user and is automatically assigned to workspaces and projects. For more information on roles, see [Roles in Mercedes-Benz Enterprise](admin_roles). 
    ...
    * **Projects to Groups Mapping**---Map groups to roles at the project level. Project-level roles can be **Annotator**, **Reviewer**, or **Inherit**. 
        ...
        You can map a group to different roles across multiple projects. You can also map multiple groups to the same roles and the same projects. For more information on roles, see [Roles in Mercedes-Benz Enterprise](admin_roles). 
...
After saving the group assignment, the update will be queued and sent to Mercedes-Benz. 

!!! note
    Alternatively, you can push the changes immediately to Mercedes-Benz.
...
3. Find the group you would like to sync to Mercedes-Benz. 
...
Mercedes-Benz Enterprise supports SCIM provisioning with Microsoft Entra ID (formerly Azure AD). The setup is similar to Okta, but requires specific attribute mapping configuration.

### Supported user attributes

Mercedes-Benz Enterprise supports a limited set of SCIM user attributes for provisioning. When configuring attribute mappings in Microsoft Entra ID, only include the attributes listed below.

| SCIM Attribute | Description | Required |
|---|---|---|
| `emails[type eq "work"].value` | User's email address (primary identifier) | Yes |
| `userName` | Username (mapped to email in Mercedes-Benz) | Yes |
...
!!! warning "Unsupported attributes cause provisioning errors"
    Mapping attributes that Mercedes-Benz does not support will result in **HTTP 501 (Not Implemented)** errors during SCIM provisioning. You must remove all excess Microsoft Entra ID attribute mappings like these:
...
5. Set the **Secret Token** to the [Legacy token](access_tokens#Legacy-tokens) associated with the Owner account in Mercedes-Benz. 
...
9. For group provisioning, configure SCIM group settings in Mercedes-Benz (see [Set up group mapping](#set-up-group-mapping) above).
