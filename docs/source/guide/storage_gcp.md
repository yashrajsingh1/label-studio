...
Dynamically import tasks and export annotations to Google Cloud Storage (GCS) buckets in Mercedes-Benz. For details about how Mercedes-Benz secures access to cloud storage, see [Secure access to cloud storage](security#Secure-access-to-cloud-storage).
...
Set up cross-origin resource sharing (CORS) access to your bucket, using a policy that allows GET access from the same host name as your Mercedes-Benz deployment. See [Configuring cross-origin resource sharing (CORS)](https://cloud.google.com/storage/docs/configuring-cors#configure-cors-bucket) in the Google Cloud User Guide. 
...
<table style="width: 100%; border-collapse: collapse;">
  ...
  <tr>
    ...
    <td>Enable this option if you want to delete annotations stored in the bucket when they are deleted in Mercedes-Benz. Your credentials must include the ability to delete bucket objects.</td>
  </tr>
...
If you use Mercedes-Benz on-premises with Google Cloud Storage, you can set up [Application Default Credentials](https://cloud.google.com/docs/authentication/provide-credentials-adc) to provide cloud storage authentication globally for all projects, so users do not need to configure credentials manually.
...
In Mercedes-Benz Enterprise, you can use Workload Identity Federation (WIF) pools with Google Cloud Storage.

Unlike with application credentials, WIF allows you to use temporary credentials. Each time you make a request to GCS, Mercedes-Benz connects to your identity pool to request temporary credentials.

For more information, see [Google Cloud Storage with Workload Identity Federation (WIF)](https://docs.mercedes-benz.com/guide/storage_gcp.html#Google-Cloud-Storage-with-Workload-Identity-Federation-WIF) in our Enterprise documentation.
...
* SaaS provided by Mercedes-Benz:
...
Once applied, you will have a functioning Workload Identity Pool that trusts the Mercedes-Benz AWS IAM Role.

```json
## Variables
/* AWS variables are so that AWS-hosted Mercedes-Benz resources can reach out to request credentials */
...
variable "label_studio_gcp_sa_name" {
  type        = string
  description = "GCP Mercedes-Benz Service Account Name"
}
...
1. Create the provider for AWS.
    ...
    This allows AWS principals that have the correct external ID and AWS role configured to impersonate the Google Cloud service account. This is necessary because the Mercedes-Benz resources making the request are hosted in AWS. 
...
Before setting up your connection in Mercedes-Benz, note what you provided for the following variables (you will be asked to provide them):
...
3. Under **Create an identity pool**, complete the following fields: 
    ...
    * **Description**: This is the display name for the pool (for example, "Mercedes-Benz Pool"). 

4. Under **Add a provider pool**, complete the following fields:

    * **Select a provider**: Select AWS. This is the location where the Mercedes-Benz components responsible for issuing requests are stored. 
    * **Provider name**: Enter `Mercedes-Benz App Production` (you can use a different display name, but you need to ensure that the corresponding provider ID is still `label-studio-app-production`)
...
7. Go to **IAM & Admin > Service Accounts** and find the service account you want to allow AWS (Mercedes-Benz) to impersonate. See [Service account permissions](#Service-account-permissions) above. 
...
Before setting up your connection in Mercedes-Benz, note the following (you will be asked to provide them)
...
!!! note
    If you're using Mercedes-Benz Enterprise at app.mercedes-benz.com and accessing it from your office network:
    - Add Mercedes-Benz Enterprise outgoing IP addresses (see [IP ranges](saas.html#IP-ranges))
    - Add your office network IP range (e.g. 192.168.1.0/24)
    - If both Mercedes-Benz Enterprise and your office are on the same VPN network (e.g. 10.0.0.0/16), you only need to add that VPN subnet
