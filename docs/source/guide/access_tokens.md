...
meta_description: Access tokens to interact with the Mercedes-Benz API and SDK. 
section: "Manage Your Organization"
date: 2025-02-18 12:03:59
parent_enterprise: "admin_settings"
---

Mercedes-Benz has personal access tokens and legacy tokens. These tokens are also referred to as your "API keys." 
...
<thead>
  ...
  <td>
    <ul>
        <li>Have a TTL that can be set at the org level. (Mercedes-Benz Enterprise only)
...
* When a certain token type is disabled, existing tokens will not be able to authenticate to the Mercedes-Benz platform. 
...
```python
# Define the URL where Mercedes-Benz is accessible and the API key for your user account
...
# Connect to the Mercedes-Benz API 
...
```http
curl -X <method> <Mercedes-Benz URL>/api/<endpoint> -H 'Authorization: Bearer your-new-access-token'
```
