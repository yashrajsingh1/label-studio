...
All labeling activities in Mercedes-Benz occur in the context of a project. After you [create a project](setup_project.html#Create-a-project) and [import data](tasks.html), set up the labeling interface and labeling configuration for your project. This setup process is essential to your labeling project.

!!! error Enterprise
    Does your project need a fully customized interface? 
    The UI engine on Mercedes-Benz Enterprise makes it possible to create programmable and embeddable interfaces for AI and agents. [Learn more](https://mercedes-benz.com/programmable-ui/).

## Set up the labeling interface for your project

Configure the labels and task type for annotators using the templates included with Mercedes-Benz or by defining your own combination of tags to set up the labeling interface for your project.

1. Select a template from the [available templates](/templates) or customize one.
2. Mercedes-Benz automatically selects the field to label based on your data. If needed, modify the selected field.
...
!!! error enterprise
    Mercedes-Benz Enterprise has the option include plugins as part of your labeling interface. Plugins allow you to use Javascript to extend your interface and customize annotator workflows. For more information, see [Plugins](https://docs.mercedes-benz.com/guide/plugins).
...
1. In Mercedes-Benz, open the project you want to modify.
2. Click **Settings**.
...
You can customize a [labeling config template](/templates) or use a custom configuration that you create from scratch using [tags](/tags). If you create a custom configuration that might be useful to other Mercedes-Benz users, consider [contributing the configuration as a template](https://github.com/Mercedes-Benz/label-studio/tree/develop/label_studio/annotation_templates).

The labeling configuration for a project is an XML file that contains three types of tags specific to Mercedes-Benz.
...
You can define the labeling configuration in a `config.xml` file and initialize a specific project in Mercedes-Benz with that file.
