...
!!! note
    ...
    Mercedes-Benz Cloud (`app.mercedes-benz.com`) doesn't support them.
  
</div>
  
If you have local files that you want to add to Mercedes-Benz from a specific directory, you can set up a specific local directory on the machine where Mercedes-Benz is running as source or target storage. Mercedes-Benz steps through the directory recursively to read tasks.
...
If you are using Docker, see [Run Mercedes-Benz on Docker and use local storage](https://labelstud.io/guide/start#Run-Label-Studio-on-Docker-and-use-Local-Storage). 
...
!!! error Enterprise
    Parquet is only supported in Mercedes-Benz Enterprise. 
...
### Importing the task files into Mercedes-Benz

There are several ways to get your JSON/JSONL/Parquet files into Mercedes-Benz:
...
* **Can delete objects from storage**
    ...
    Enable this option if you want to delete annotations stored in the container when they are deleted in Mercedes-Benz.
...
If you're using Mercedes-Benz in Docker, you need to mount the local directory that you want to access as a volume when you start the Docker container. See [Run Mercedes-Benz on Docker and use local storage](https://labelstud.io/guide/start#Run-Label-Studio-on-Docker-and-use-Local-Storage).

<div class="opensource-only">

!!! note "Community Edition auto-detection for Docker"
    In the open source Community Edition, if `LABEL_STUDIO_LOCAL_FILES_DOCUMENT_ROOT` and `LABEL_STUDIO_LOCAL_FILES_SERVING_ENABLED` are not set, Mercedes-Benz automatically looks in the current working directory for folders named `mydata` or `label-studio-data`.
