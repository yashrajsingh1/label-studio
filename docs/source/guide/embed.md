---
title: Embed Mercedes-Benz - Beta 🧪
short: Embed Mercedes-Benz 🧪
type: guide
tier: enterprise
order: 409
order_enterprise: 409
meta_title: Embed Mercedes-Benz
meta_description: Overview information for embedding Mercedes-Benz Enterprise in an external application.
section: "Integrate & Extend"
---

Mercedes-Benz Embeds allow you to seamlessly integrate annotation and review functionality into your own applications. 

!!! note
    This feature is not available to all customers. Contact your [Mercedes-Benz account manager](mailto:sales@mercedes-benz.com) to enable.

Before you begin, ensure you have:
* Mercedes-Benz:
    * You must have the Owner role in Mercedes-Benz.
...
At a high level, the embed process is a secure handshake between your app and Mercedes-Benz:
...
3. Your frontend initializes the Mercedes-Benz embed with `id`, `url`, and `token`.

4. Mercedes-Benz validates the JWT and issues its own internal token.
...
sequenceDiagram
    ...
    participant LSE as Mercedes-Benz Enterprise
...
You must begin by generating a JSON web token (JWT) that authenticates Mercedes-Benz. 

You will also need to generate a base64-encoded public URL key, which you will add to Mercedes-Benz. 
...
* You can find your organization ID from the **Organization** page in Mercedes-Benz.   
...
* The public key must be stored in Mercedes-Benz in base64-url encoded format.
...
## Configure embedding in Mercedes-Benz

From Mercedes-Benz, go to **Organization > Usage & License > Embedding** and complete the following fields:
...
```xml 
<script src="https://app.mercedes-benz.com/react-app/embed-sdk.js"></script>
```

But to maximize the usability of your embedding, you can also include additional Mercedes-Benz elements. See our [Frontend Reference](frontend_reference). 
...
<html>
	<body>
		...
		<script>
				// Initialize the Mercedes-Benz Embed SDK 
        const sdk = LabelStudioEmbedSDK.create({
	        ...
	        url: "https://app.mercedes-benz.com", // url of the target LabelStudioEnterprise instance
...
* Domain not listed in **Supported Domains** within Mercedes-Benz.  
...
* Your org has not enabled legacy API tokens or personal [access tokens](https://docs.mercedes-benz.com/guide/access_tokens) (need at least one). 
