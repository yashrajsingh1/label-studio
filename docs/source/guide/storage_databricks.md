...
In Mercedes-Benz Enterprise, you can connect to Databricks Unity Catalog (UC) volumes for your source and target storage. 

For more information, see [Databricks Files (UC Volumes)](https://docs.mercedes-benz.com/guide/storage_databricks) in our Enterprise documentation.

</div>

<div class="enterprise-only">

Connect Mercedes-Benz Enterprise to Databricks Unity Catalog (UC) Volumes to import files as tasks and export annotations as JSON back to your volumes. This connector uses the Databricks Files API and operates only in proxy mode ([presigned URLs](storage#Pre-signed-URLs-vs-Storage-proxies) are not supported by Databricks).
...
When configuring storage in Mercedes-Benz, you will be asked for your access token.  
...
When configuring storage in Mercedes-Benz, you will be asked for the following:
...
!!! note 
    ...
    For Service Principal authentication, Mercedes-Benz automatically acquires and refreshes OAuth access tokens (~1 hour lifetime). No manual token rotation needed.
...
!!! note "URI schema"
    To reference Databricks files directly in task JSON (without using source storage), use Mercedes-Benz’s Databricks URI scheme:
...
!!! note "Troubleshooting"
    ...
    - If previews work but media fails to load, confirm proxy mode is allowed for your organization in Mercedes-Benz (**Organization > Usage & License > Features**) and network egress allows Mercedes-Benz to reach Databricks.


!!! warning "Proxy and security"
    This connector streams data **through the Mercedes-Benz backend** with HTTP Range support. Databricks does not support presigned URLs, so this option is also not available in Mercedes-Benz.
...
When file deletion is enabled, if you delete an annotation in Mercedes-Benz (via UI or API), Mercedes-Benz will also delete the corresponding exported JSON file from your target storage for this storage connection. 
