...
meta_description: Template for labeling multivariate and simple time series data with Mercedes-Benz for your machine learning and data science projects.
---
...
```
The `valueType="url"` parameter means that Mercedes-Benz expects links to CSV files in JSON-formatted tasks. The `timeColumn` parameter specifies the column in your dataset to use as the X-axis for time. If you don't specify a `timeColumn`, Mercedes-Benz uses incremental integer values as the X-axis: `0, 1, 2, ...`.
...
Mercedes-Benz supports several input types for time series:
...
Then, create a JSON file that references a URL for the CSV file to upload to Mercedes-Benz:
```json
...
All tasks in Mercedes-Benz are stored in JSON and this is the native format for Mercedes-Benz. 

- `valueType="url"`

  When you use `valueType="url"` for TimeSeries tag and import a CSV file, Mercedes-Benz automatically creates a JSON task with the body like this example: 
...
- `valueType="json"`
  ...
  You can also use Mercedes-Benz JSON format directly by creating and import JSON structured like the following example, where each key specifies the time and channels: 
...
Annotators add labels to time series tasks. Mercedes-Benz represents each completed annotation with a JSON structure. Each annotation has a `result` field that looks like the following example for time series labeling projects:
