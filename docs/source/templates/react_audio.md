...
!!! error Enterprise
    This template and the `ReactCode` tag can only be used in Mercedes-Benz Enterprise.

    For more information, see [Programmable & Embeddable Interfaces](https://mercedes-benz.com/programmable-ui/). 

## Labeling configuration

This example uses three main Mercedes-Benz components working together:
...
The transcript data is stored as an array of segment objects, each containing `speaker`, `start`, `end`, and `text` fields. The `ReactCode` component manages this data structure and persists changes through Mercedes-Benz's region API.
