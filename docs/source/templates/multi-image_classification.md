...
meta_description: Template for classifying multiple images at once with Mercedes-Benz for your machine learning and data science projects.
---
...
<Image name="img-left" value="$image1"/>
```
The `value` parameter specifies a JSON key of `"image1"` in the Mercedes-Benz JSON task that must be imported with URLs to both images. 
...
<Image name="img-right" value="$image2"/>
```
The `value` parameter specifies a JSON key of `"image2"` in the Mercedes-Benz JSON task that must be imported with URLs to both images. 
