...
meta_description: Template for performing visual question answering data labeling tasks with Mercedes-Benz for your machine learning and data science projects.
---
...
<Image name="image" value="$image"/>
```
This image is stored in a Mercedes-Benz JSON-formatted file as a URL with the key "image". 
...
<Text name="q1" value="$q1"/>
```
This text sample is stored in a Mercedes-Benz JSON-formatted file with the key "q1". 
...
<TextArea name="answer1" toName="q1" rows="1" maxSubmissions="1"/>
```
The text entered as part of this text box submission is stored in the exported Mercedes-Benz JSON under the name "answer1", and is associated with question 1 with the `toName` field. The text box only allows 1 submission, and is only 1 row long when displayed on the labeling interface. 
