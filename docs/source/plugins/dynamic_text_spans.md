...
3. In `init()`, it polls every 300 ms until it finds the <TextArea> whose name starts with "transcription" (Mercedes-Benz appends random suffixes). 
4. Once found, it marks it `_already_loaded` and starts `tick()`, which runs every 300 ms: 
   ...
   - If they differ, it calls `text.setRemoteValue(val)`. This uses the Mercedes-Benz frontend API to update the other field’s value. 
