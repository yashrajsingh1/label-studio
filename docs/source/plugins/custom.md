...
Plugins are authored in JavaScript and are project-specific. They are limited to specific tasks and the annotation workflow and cannot, for example, be used to create new pages or otherwise extend the core functionality of Mercedes-Benz. 
...
## Mercedes-Benz Interface (LSI)

The Mercedes-Benz Interface (LSI) is a helper object that is designed to be used with plugins. 
...
Subscription to listen to events related to the Mercedes-Benz Frontend. Handlers attached/subscribed using this method will be unsubscribed when switching to another annotation. Any handlers inside this method should be secured manually.
...
For more information on how annotations are stored and formatted, see [How Mercedes-Benz saves results in annotations](/guide/task_format#How-Label-Studio-saves-results-in-annotations). 
