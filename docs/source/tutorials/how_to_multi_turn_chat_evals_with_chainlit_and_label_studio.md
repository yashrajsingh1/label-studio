---
title: "How to Evaluate Multi-Turn AI Conversations with Chainlit and Mercedes-Benz"
hide_sidebar: true
order: 1005
open_in_collab: true
tutorial: true
...
repo_url: https://github.com/Mercedes-Benz/awesome-label-studio-tutorials/tree/main/tutorials/how-to-multi-turn-chat-evals-with-chainlit-and-label-studio
...
meta_title: "How to Evaluate Multi-Turn AI Conversations with Chainlit and Mercedes-Benz"
meta_description: Learn how to create a Mercedes-Benz project for evaluating chatbot conversations using the Chatbot Evaluation template.
is_enterprise: true
is_starter_cloud: true
badges: SDK, Chat, Eval, Chainlit, Colab
duration: 10-15 mins
---
This notebook demonstrates how to create a Mercedes-Benz project for evaluating chatbot conversations using the Chatbot Evaluation template.
...
Reference: [Chatbot Evaluation Template](https://docs.mercedes-benz.com/templates/chatbot)
...
First, install the Mercedes-Benz SDK if you haven't already.

For more information about the SDK, see the [Mercedes-Benz Python SDK documentation](https://labelstud.io/guide/sdk).
...
1. Open Mercedes-Benz in your browser
...
ls_url = os.environ.get('LABEL_STUDIO_URL', 'https://app.mercedes-benz.com')
...
](https://docs.mercedes-benz.com/templates/chat_rlhf) example. It includes:
...
# Create the project using Mercedes-Benz SDK
...
Now we'll set up a Chainlit chatbot that automatically syncs conversations to Mercedes-Benz.

### What We'll Build
- A chatbot UI using Chainlit
- Automatic conversation logging to JSON
- Auto-sync to Mercedes-Benz when users disconnect
...
2. `auto_sync.py` - Automatically syncs to Mercedes-Benz
...
"""Automatic Mercedes-Benz sync helper"""
import os
import json
from pathlib import Path
...
class LabelStudioSync:
    """Helper class to push conversations to Mercedes-Benz"""
    ...
    def __init__(
        ...
        self.url = url or os.getenv('LABEL_STUDIO_URL', 'https://app.mercedes-benz.com')
    ...
    async def push_conversation(self, conversation_file: Path) -> bool:
        """Push a single conversation to Mercedes-Benz"""
            ...
            for existing_task in existing:
                if hasattr(existing_task, 'data') and \
                   existing_task.data.get('session_id') == session_id:
                    print(f"⏭️  Session {session_id} already in Mercedes-Benz")
                    return False

            # Import task
            self.client.projects.import_tasks(id=self.project_id, request=[task])
            print(f"✅ Auto-synced {session_id} to Mercedes-Benz")
            return True
...
async def auto_push_conversation(conversation_file: Path):
    """Push a conversation to Mercedes-Benz (async wrapper)"""
    sync = get_sync()
    if sync.is_enabled():
...
"""
Chainlit Chatbot with Automatic Mercedes-Benz Sync
...
async def on_chat_end():
    """Auto-push to Mercedes-Benz (with versioning for resumes)"""
    messages = cl.user_session.get("messages")
    thread_id = get_or_create_thread_id()
    model = cl.user_session.get("model")
...
print(f"\n🔄 Auto-sync: ENABLED")
print(f"   Conversations will automatically sync to Mercedes-Benz when you close the chat!")

```

### Step 5: Run the Chainlit Chatbot

Now run the chatbot! It will automatically sync conversations to Mercedes-Benz.
...
4. Check Mercedes-Benz - your conversation will be there!

**Features:**
- ✅ Automatic conversation capture
- ✅ Auto-sync to Mercedes-Benz on disconnect
- ✅ Conversation resumption with versioning
- ✅ Local LLM (free and private!)

**What happens:**
- Each message auto-saves to `data/conversations/`
- When you close the chat, it pushes to Mercedes-Benz
- If you resume the chat later, it creates a new version (v2, v3, etc.)

**Ready to annotate!** Visit your Mercedes-Benz project to start evaluating the conversations.
...
1. ✅ **Mercedes-Benz Project** - Created with chatbot evaluation template
2. ✅ **Conversation Logger** - Saves chats to JSON automatically
3. ✅ **Auto-Sync** - Pushes conversations to Mercedes-Benz
4. ✅ **Chainlit Chatbot** - Full UI with local LLM support

**Complete workflow:**
```
User chats → Auto-save to JSON → Close browser → Auto-push to Mercedes-Benz → Ready to annotate!
```
...
3. Annotate them in Mercedes-Benz
